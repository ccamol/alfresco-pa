...
public class CheckLists extends EntityManager<CheckList>{

	public CheckLists(){
		super(CheckList.class);
	}
	
	public Response<Boolean> checkComplete(Integer nodeType, Integer nodeId){
		Response<Boolean> response = new Response<Boolean>();
		response.setResult(true);
		
		Response<List<CheckListResultSet>> list = checkDocuments(nodeType, nodeId);
		if(list.getStatus()>-1 && list.getResult()!=null && list.getResult().size()>0){
			for(CheckListResultSet each : list.getResult()){
				if(!each.getCheck()){
					System.out.println("PENDIENTE ESTE DOCUMENTO: ");
					System.out.println(each.getUuid());
	...
	public Response<List<CheckListResultSet>> checkDocuments(Integer nodeType, Integer nodeId){
		...
		List<CheckList> documentsToCheck = this.getListByFilter(filter).getResult();
...
//				item.setOwner((String) nodeService.getProperty(resultSet.getNodeRef(0), ContentModel.PROP_MODIFIER));
				item.setOwner((String) nodeService.getProperty(resultSet.getNodeRef(0), ContentModel.PROP_CREATOR));
