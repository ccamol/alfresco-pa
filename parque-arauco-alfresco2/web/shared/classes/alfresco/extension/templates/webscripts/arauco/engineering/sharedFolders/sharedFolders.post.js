...
//		logger.log("Node description: " + childrens.result.description);
        ...
        query='@sys\\:node-uuid:' + uuidSelectedFolder;
        resultSet = search.luceneSearch(query);
        sourceNodeRef = resultSet[0];
        logger.log("sourceNodeRef: " + sourceNodeRef);

        query2='@sys\\:node-uuid:' + CompartidaResultUUID;
        resultSet = search.luceneSearch(query2);
        targetParentRef = resultSet[0];
        logger.log("targetParentRef: " + targetParentRef);
        
        var copyService = CopyServiceImpl.copy(sourceNodeRef,targetParentRef,null,null,true);
