...
function popupBuild(message , typePopup){
	...
	}]).addClass(cssPopup).attr('id', n);
	$('.modal-backdrop').attr('id','delete');

	if(typePopup == 2 || typePopup == 3 || typePopup == 4){
		$('.modal-footer').remove();		
		$(".close").click( function(){
			...
			$('#delete').remove();
			$('.modal-backdrop').attr('id','delete');
			if($('#projectMallThirdPartyArchitecture').length > 0){
			scrollTo('associatedThirdPartyArchitectureDocuments');								
			}
		});
	}	
}
...
function openWaiting(){
	$('body').append('<div class="waiting"><p class="cargando">Cargando...</p></div>');	
}
