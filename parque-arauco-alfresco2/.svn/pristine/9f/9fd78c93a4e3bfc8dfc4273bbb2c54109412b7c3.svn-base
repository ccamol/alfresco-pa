...
$(document).ready(function() {	
	...
	var data = {};
	data["url"] = "/arauco/awardApplicantTender/getListByFilter";
...
function deleteDocument(uuid , id){
	var validate = validateStatusStages();
	if(validate){
		bootbox.confirm("Se eliminará el documento. ¿Desea continuar?", "Cancelar", "Aceptar", function(result) {
			if(result==true){
				$.ajax({
					url:'/share/page/dashlets/deleteDocumentApplicant',
					type: "get",
					dataType: "json",
					data: {
						uuid : uuid,
					...
					success:   function(data) {
						if(data.status == true){
							popupSuccess("Eliminación realizada con éxito"); 
							getRequestDocumentApplicant();
						}else{
							popupAlert("No se pudo eliminar el documento");
						}
					}
				});
			}
...
function getConsult(){
	$.ajax({
		url:'/share/page/dashlets/getawardEvaluationConsulting',
		type: "get",
		dataType: "html",
		async : false,
		data: {
...
function viewQuestions(idCategory , cont){
	$.ajax({
		url:'/share/page/dashlets/awardEvaluationQuestions',
		type: "get",
		dataType: "html",  
		async: false,
		data : {
...
function getSharedDocumentApplicant(){
	$.ajax({
...
function getRequestDocumentApplicant(){
	$.ajax({
		url:'/share/page/dashlets/requestedDocumentOfferApplicant',
		type: "get",
		dataType: "html",
		async : false,
		data: {
...
function validateConsultForm(category, question){
	var status=false;
	var stageStatus =  getStagesTenderDetails(6);
	...
	}else if(stageStatus.stageStatus != 1){
		$("#createConsult").prop( "disabled", true );
...
function updateDocument(uuid, documentName) {
	...
	if(validate){
		$.ajax({
			url:'/share/page/dashlets/updateContentDocumentApplicant?uuid=' + uuid,
			type: "get",
			dataType: "html",
			async: false,
			success: function(data) {
